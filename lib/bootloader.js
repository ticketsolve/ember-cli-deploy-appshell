...
(function() {
  ...
  function cacheBust() {
    log('- Busting cache');
    var findCSRFMetaTag = function(doc, callback) {
      var tags = doc.getElementsByTagName('meta');
      for (var i = 0; i < tags.length; i++) {
        if (tags[i].getAttribute('name') === 'csrf-token') {
          callback(tags[i]);
          break;
        }
      }
    };
    var replaceCSRFMetaTag = function(source, destination) {
      var token;
      findCSRFMetaTag(source, function(tag) {
        token = tag.getAttribute('content');
      });
      findCSRFMetaTag(destination, function(tag) {
    ...
    getShadowIndex(function(html) {
      ...
      // TICKETSOLVE CUSTOM: We need to replace the CSRF token back
      // into index.html after replacing with appshell.html
      replaceCSRFMetaTag(doc, document);
